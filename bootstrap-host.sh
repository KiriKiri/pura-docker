...
PURA_IMAGE=${PURA_IMAGE:-puracore/purad}
...
docker kill purad-node 2>/dev/null || true
docker rm purad-node 2>/dev/null || true
stop docker-purad 2>/dev/null || true

# Always pull remote images to avoid caching issues
if [ -z "${PURA_IMAGE##*/*}" ]; then
    docker pull $PURA_IMAGE
fi

# Initialize the data container
docker volume create --name=purad-data
docker run -v purad-data:/pura --rm $PURA_IMAGE pura_init

# Start purad via upstart and docker
curl https://raw.githubusercontent.com/puracore/docker-purad/master/upstart.init > /etc/init/docker-purad.conf
start docker-purad

set +ex
echo "Resulting pura.conf:"
docker run -v purad-data:/pura --rm $PURA_IMAGE cat /pura/.pura/pura.conf